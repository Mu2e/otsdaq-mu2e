...
| 13 | RX_FCS_BAD | an accepted 0x78-framed EVB frame arrived with an FCS that does not match the CRC-32 of its words (same span the sender covers: destination word through the last payload/pad word). Added 2026-09-16 (the RX CRC engine had been commented out for years; 0x9590 is dead in EVB3 builds). The frame was already stored when this fires, so it flags corruption after the fact. 4-byte-shifted (0x33) frames are not checked. Not yet run in hardware (first bitfile pending the 2026-09-16 sim run 12). ILA: `evb_rx_ila` trigger `rx_fcs_bad == 1` | **2026-09-22:** on builds 0xd6091797 through 0xd6092291 this bit is a false alarm: the checker's CRC engine is cleared by a one-clock asynchronous clear that does not leave it at all-ones in the built design, so the first frame after any clear compares wrong (the wire FCS was correct in every ILA capture). Fixed in the build after 0xd6092291: two-clock clear with the enable held off. Software keeps those four builds on its exception list; from the next build on, bit 13 means a real FCS mismatch.
